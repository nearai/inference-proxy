...
- Optional: `CLOUD_API_URL` (enables `sk-` API key auth via cloud-api and requires `CLOUD_API_USAGE_TOKEN` plus a persistent `CLOUD_API_USAGE_OUTBOX_DIR` mount), `LOG_FORMAT=json` (structured JSON logs)
...
- `CLOUD_API_URL` enables two coupled features: (1) `sk-live-`/`sk-test-` API key validation via `POST /v1/check_api_key`, (2) durable usage reporting via `POST /v1/internal/usage`. Startup fails when only one of `CLOUD_API_URL` and `CLOUD_API_USAGE_TOKEN` is configured.
- Usage reporting is split between `proxy.rs` and `usage_outbox.rs`: `enqueue_usage_report()` fsyncs an attributed event under `CLOUD_API_USAGE_OUTBOX_DIR` (default `/var/lib/inference-proxy/usage-outbox`) before the background worker sends it with the shared token. The token is never persisted. The worker removes events only after 2xx, retries transient/systemic failures with capped full jitter, and replays after restart. Cloud API de-duplicates by organization + inference ID.
- Mount `CLOUD_API_USAGE_OUTBOX_DIR` on a persistent volume used by exactly one proxy instance. Monitor `inference_proxy_usage_outbox_pending`, delivery outcomes, enqueue write errors, and quarantined events.
- **MODEL_NAME must exactly match `model_name` in cloud-api's model table** — cloud-api does not check model aliases. A mismatch returns 404 and remains in the outbox until configuration converges.
