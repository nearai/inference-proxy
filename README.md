...
| `VLLM_BACKEND_CONTINUATION_MIN_INPUT_TOKENS` | No | `0` (off) | Let a completion-confirmed recent chat prefix at or above this estimated size wait briefly for its exact backend when normal admission would reject for budget or engine-queue pressure; requires admission and conversation affinity |
| `VLLM_BACKEND_CONTINUATION_MAX_AGE_SECS` | No | `180` | Maximum age of the successful completion that makes the next matching prefix eligible (hard maximum `180`) |
| `VLLM_BACKEND_CONTINUATION_MAX_WAIT_MS` | No | `1000` | Maximum local wait for eligible continuations (hard maximum `5000`); cold work still fails fast |
| `VLLM_BACKEND_CONTINUATION_MAX_WAITERS` | No | `8` | Process-local waiting-task cap (hard maximum `8`) |
| `VLLM_BACKEND_CONTINUATION_MAX_BUFFERED_BYTES` | No | `67108864` | Aggregate raw request bytes attributed to waiting tasks; the queue stores metadata, not another body copy |
| `VLLM_BACKEND_CONTINUATION_MAX_ESTIMATED_TOKENS` | No | `8000000` | Aggregate estimated prompt-token cap across waiting tasks |
