...
| `VLLM_PROXY_CAPACITY_REQUESTS_PER_MINUTE` | No | `0` (not declared) | Requests per minute declared in the models document's `capacity`; the concurrency entry is the active admission maximum (the environment bootstrap value until AppConfig supplies a valid policy) |
...
| `AWS_APPCONFIG_APPLICATION` | No | unset | AWS AppConfig application read through the local Agent. Set together with environment, profile, and target to enable hot admission-policy updates. |
| `AWS_APPCONFIG_ENVIRONMENT` | No | unset | AppConfig environment for this independently operated proxy group. |
| `AWS_APPCONFIG_PROFILE` | No | unset | AppConfig configuration profile for this independently operated proxy group. |
| `AWS_APPCONFIG_TARGET` | No | unset | Exact target required in the runtime document, preventing one proxy group from adopting another group's policy. |
| `AWS_APPCONFIG_AGENT_URL` | No | `http://127.0.0.1:2772` | Local AWS AppConfig Agent base URL. Startup and request serving do not depend on this endpoint; failures retain the current environment or last-known-good policy. |
| `AWS_APPCONFIG_REFRESH_SECS` | No | `5` | How often the proxy reconciles the Agent's local cache. This does not control or call the remote AWS poll. |
| `AWS_APPCONFIG_AGENT_ACCESS_TOKEN` | No | unset | Optional bearer token for the local Agent. Treated as sensitive and never logged. |
