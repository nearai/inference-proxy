...
| `VLLM_BACKEND_LONG_CONTEXT_ABOVE_TOKENS` | No | `0` (off) | Estimated input tokens (cloud-api's bytes/4 heuristic with its 1.2 safety factor; `prompt` token ids counted exactly) above which a request goes to the long-context tier. A tier with no healthy backend falls back to the other one; a full one refuses with 429 |
