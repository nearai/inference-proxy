...
| `VLLM_PROXY_DISCOUNT_TO_USER` | No | empty (list price) | Gateway mode: the lane's discount off the list price, a fraction in `[0, 1)` with at most four decimal places (`0.2` = 20 % off; empty or `0` = none; an invalid value fails startup). Published as `discount_to_user` on the models document entry and sent as `discount_to_user` on every usage report, so the advertised and the billed price come from one setting |
