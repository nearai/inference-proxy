...
| `VLLM_BACKEND_LONG_CONTEXT_ABOVE_TOKENS` | No | `0` (off) | Estimated input tokens above which a request goes to the long-context tier, using a variant of cloud-api's own routing estimate (text, tool calls and tool definitions at bytes/4 with `VLLM_BACKEND_LONG_CONTEXT_SAFETY_FACTOR`, plus a flat cost per media part and per-message template overhead, plus the reserved output window when `VLLM_BACKEND_LONG_CONTEXT_COUNT_OUTPUT_RESERVE` is set; `prompt` token ids counted exactly). By default a tier with no healthy backend falls back to the other one (`VLLM_BACKEND_TIER_STRICT` turns that off); a full one refuses with 429. Not combinable with `FUSION_ENABLED` or `WEB_CONTEXT_SEARCH_URL` |
| `VLLM_BACKEND_LONG_CONTEXT_SAFETY_FACTOR` | No | `1.2` | Multiplies the byte-estimated text portion of the long-context estimate — cloud-api's own factor, kept as the default deliberately, since guessing high just means a prefill on the long tier instead of the base one (both run the same engine). Must be a positive, finite number |
| `VLLM_BACKEND_LONG_CONTEXT_COUNT_OUTPUT_RESERVE` | No | `false` | Whether the caller's reserved output window (`max_completion_tokens` / `max_tokens`) counts toward the long-context tier decision. Off by default: both tiers run the same engine, so the output window has no bearing on prefill cost, and counting it was placing small-prompt, large-`max_tokens` requests on the long tier for no reason. `1` reproduces cloud-api's own behaviour, where it guards a real capacity limit — the rollback |
