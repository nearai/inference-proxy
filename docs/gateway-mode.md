...
| `VLLM_BACKEND_CONTINUATION_MIN_INPUT_TOKENS` | `0` (off) | Enables bounded waiting only for completion-confirmed recent prefixes at or above this estimated prompt size. Requires admission and conversation affinity. Use `100000` for the initial GLM-5.3 Flash canary. |
| `VLLM_BACKEND_CONTINUATION_MAX_AGE_SECS` / `_MAX_WAIT_MS` | `180` / `1000` | A successful matching completion must be this recent; eligible work may wait this long for budget or engine-queue pressure to clear. Hard maxima are 180 seconds and 5 seconds. |
| `VLLM_BACKEND_CONTINUATION_MAX_WAITERS` / `_MAX_BUFFERED_BYTES` / `_MAX_ESTIMATED_TOKENS` | `8` / `67108864` / `8000000` | Independent process-local bounds on waiting tasks and their attributed request demand. |
