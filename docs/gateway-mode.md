...
| `VLLM_PROXY_ADMISSION_QUEUE_SATURATED_AT` | `4` | Engine-reported queue depth at or above which a backend counts as saturated for placement and the fleet-wide queue refusal below. Default `1` (any non-zero queue); raise it for engines that run chunked prefill, where a shallow queue is normal while slots are still free. |
