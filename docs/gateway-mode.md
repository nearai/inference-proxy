...
| `VLLM_PROXY_ADMISSION_TTFT_P95_MAX_MS` | `30000` | Refuse new work while the lane's own time-to-first-chunk p95 over the last minute is above this. |
| `VLLM_PROXY_ADMISSION_BACKPRESSURE_SECS` | `10` | Refuse new work while every healthy backend rejected at engine admission within this window. |
| `VLLM_BACKEND_CONNECT_FAILOVER` | `1` | A backend that refuses the connection (host down, proxy restarting) costs the request nothing: it is re-sent once to another healthy backend. Never on an HTTP error. |
...
1. **Observed overload.** Two signals the gateway measures on its own traffic:
   the time-to-first-chunk p95 over the last minute (at least 5 samples) above
   `VLLM_PROXY_ADMISSION_TTFT_P95_MAX_MS`, or every healthy backend having
   answered an engine admission rejection within
   `VLLM_PROXY_ADMISSION_BACKPRESSURE_SECS`. Either refuses new work until the
   signal ages out (reasons `ttft` and `backend_queue`).
...
3. **Per-host share.** `ceil(budget / healthy backends)` in flight per backend,
   ...
   pinned conversation whose host is at its share moves to the least-loaded
   host with room, and only when no host has room is the request refused
   (reason `host_share`).
...
where nothing reached the engine yet.
