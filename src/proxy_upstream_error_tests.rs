...
fn test_queue_full_maps_to_429_only_when_enabled() {
    ...
    // Priority scheduling displaces queued low-priority requests through the
    // same abort path; that is back-pressure too.
    let displaced = parse_upstream_error(
        br#"{"object":"error","message":"The request is aborted by a higher priority request.","type":"abort","code":503}"#,
    );
    assert_eq!(
        effective_error_status(503, displaced.as_ref(), true),
        StatusCode::TOO_MANY_REQUESTS
    );
